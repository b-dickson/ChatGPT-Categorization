...
def get_responses(rock1, rock2, prompt_type, anchors, model, image_folder):
    ...
    user_message = create_messages((rock1, rock2), image_folder)
    ...
    for i, pair in enumerate(pairs, start=args.start):
        ...
        try:
            response = get_responses(
                rock1, rock2, prompt_type=args.prompt_type, anchors=args.anchors, 
                model=args.model, image_folder=image_folder
            )
        ...
        try:
            ...
            rating = number_map.get(response_text.lower(), -1)
            if rating == -1:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Get rock similarity ratings using Ollama.")
    parser.add_argument(
        "--dataset",
        type=str,
        choices=["30", "360"],
