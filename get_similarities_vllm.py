...
def load_image_as_data_url(image, folder=IMAGE_FOLDER):
...
def create_multimodal_input(rocks, prompt, folder=IMAGE_FOLDER):
    # Create the text and image content for vLLM multimodal input
    content = [{"type": "text", "text": f"{prompt}\n\n{QUESTION_PROMPT}"}]
    
    for rock in rocks:
        content.append({
            "type": "image_url",
            "image_url": {"url": load_image_as_data_url(rock, folder)}
        })
...
def get_responses(llm, sampling_params, rock1, rock2, prompt_type, anchors):
    ...
    if anchors:
        # Add anchor examples
        anchor_messages = [
            create_multimodal_input((SIMILAR_ROCK1, SIMILAR_ROCK2), prompt),
            {"role": "assistant", "content": "9" if prompt_type != "reverse" else "1"},
            create_multimodal_input((DISSIMILAR_ROCK1, DISSIMILAR_ROCK2), prompt),
            {"role": "assistant", "content": "1" if prompt_type != "reverse" else "9"},
            create_multimodal_input((MEDIUM_ROCK1, MEDIUM_ROCK2), prompt),
            {"role": "assistant", "content": "5"},
        ]
        messages.extend(anchor_messages)
    
    # Add the main query
    messages.append(create_multimodal_input((rock1, rock2), prompt))
...
def extract_rating_from_text(text, number_map):
    # Clean the text and extract the rating
    text = text.strip().lower()
    
    # First try to find a direct match in the number map
    if text in number_map:
        try:
            return int(number_map[text])
        except (ValueError, TypeError):
            pass
    
    # Try to extract a single digit 1-9
    digits = re.findall(r'\b([1-9])\b', text)
    if digits:
        return int(digits[0])
    
    # If no valid rating found, return -1
    return -1
...
def main(args):
    ...
    llm = LLM(
        ...
        trust_remote_code=True,  # Required for many HF models
    )
    ...
    for i, pair in enumerate(pairs, start=args.start):
        ...
        try:
            response = get_responses(
                llm, sampling_params, rock1, rock2, 
                prompt_type=args.prompt_type, anchors=args.anchors
            )
        ...
        vllm_rating = extract_rating_from_text(response_text, number_map)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Get rock similarity ratings using vLLM with HuggingFace VLMs.")
    parser.add_argument(
        "--start",
        type=int,
        default=0,
    ...
    parser.add_argument(
        ...
        help="HuggingFace VLM model to use (default: llava-hf/llava-1.5-7b-hf). Popular options: "
             "llava-hf/llava-1.5-7b-hf, llava-hf/llava-1.5-13b-hf, "
             "OpenGVLab/InternVL2-8B, OpenGVLab/InternVL2-26B, "
             "openbmb/MiniCPM-V-2_6, microsoft/Florence-2-large, "
             "HuggingFaceM4/idefics2-8b, Qwen/Qwen2-VL-7B-Instruct",
    )
    parser.add_argument(
        "--max_model_len",
        type=int,
